...
public enum KeyNamingConvention {
    ...
    KeyNamingConvention(String name) {
     ...
     *
     * @param displayName the string representation of the naming convention
     * @return the corresponding NamingConvention enum value
     */
    static public KeyNamingConvention fromDisplayName(String displayName) {
        for (KeyNamingConvention value : KeyNamingConvention.values()) {
            if (value.getDisplayName().equals(displayName))
                return value;
        }
        return KeyNamingConvention.CAMEL_CASE;
    }

    /**
     * Returns an array of strings representing the names of the enum values in the {@link KeyNamingConvention} enum.
     *
     * @return an array of strings representing the enum names
     */
    static public String[] displayNames() {
        return Arrays.stream(KeyNamingConvention.values())
                .map(KeyNamingConvention::getDisplayName)
                .toArray(String[]::new);
    }
