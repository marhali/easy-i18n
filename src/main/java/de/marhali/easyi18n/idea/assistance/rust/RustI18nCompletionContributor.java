...
public class RustI18nCompletionContributor extends CompletionContributor {
    ...
    public RustI18nCompletionContributor() {
        extend(
            ...
            PlatformPatterns.psiElement().withParent(RsLitExpr.class),
            new CompletionProvider<>() {
                ...
                protected void addCompletions(
                    ...
                    RsLitExpr literal = findParentOfType(position, RsLitExpr.class);

                    if (literal == null || !RustEditorElementExtractor.isStringLiteral(literal)) {
                        return;
                    }
                    ...
                    String currentValue = RustEditorElementExtractor.getStringContent(literal);
                    if (currentValue == null) {
                        currentValue = "";
                    }

                    // Value range = literal content between the first and last quote
                    String rawText = literal.getText();
                    int firstQuote = rawText != null ? rawText.indexOf('"') : 0;
                    TextRange literalRange = literal.getTextRange();
                    TextRange absoluteValueRange = TextRange.create(
                        literalRange.getStartOffset() + firstQuote + 1,
