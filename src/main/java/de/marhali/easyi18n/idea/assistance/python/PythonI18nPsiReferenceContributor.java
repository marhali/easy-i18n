 ...
 * @author marhali
 */
public class PythonI18nPsiReferenceContributor extends PsiReferenceContributor {

    @Override
    public void registerReferenceProviders(@NotNull PsiReferenceRegistrar registrar) {
        registrar.registerReferenceProvider(
            PlatformPatterns.psiElement(PyStringLiteralExpression.class),
            new PythonI18nPsiReferenceProvider()
        );
    }

    private static final class PythonI18nPsiReferenceProvider extends PsiReferenceProvider {

        @Override
        public PsiReference @NotNull [] getReferencesByElement(@NotNull PsiElement psiElement, @NotNull ProcessingContext context) {
            if (!(psiElement instanceof PyStringLiteralExpression literal)) {
