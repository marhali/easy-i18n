...
public class ModuleLocalesQueryHandlerTest {
    ...
    private record Fixture(ModuleLocalesQueryHandler handler, InMemoryI18nStore store, InMemoryProjectConfigAdapter projectConfigPort) {}

    private Fixture buildFixture() {
        return buildFixture(new LocaleId("en"));
    }

    private Fixture buildFixture(LocaleId previewLocale) {
        ...
        var handler = new ModuleLocalesQueryHandler(localesOrderService, new DummyEnsureLoadedService(), store);
    ...
    @Test
    public void test_preview_locale_is_first_in_result() {
        var de = new LocaleId("de");
        var fixture = buildFixture(de);
        populateLocales(fixture, new LocaleId("en"), de, new LocaleId("fr"));

        var result = fixture.handler().handle(new ModuleLocalesQuery(MODULE_ID));

        Assert.assertEquals(de, result.iterator().next());
    }
